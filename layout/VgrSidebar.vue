...
<template>
  <nav ref="bar" class="h-auto" data-sidebar>
    <div v-bind="$attrs" class="w-64 h-full bg-white dark:bg-base-20 relative" :class="{'sticky !h-[calc(100vh-var(--top,0px))] top-[--top] z-50': sticky}" :style="{'--top': (sticky ? top : 0) +'px'}">
      <!-- Default slot -->
      <slot/>
      <!-- Shadow -->
